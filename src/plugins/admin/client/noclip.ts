...
export const NoClip = {
    toggle() {
        if (!alt.getMeta('noclip-enabled')) {
            NoClip.enable();
            alt.setMeta('noclip-enabled', true);
            alt.emitServer(AdminEvents.noclip.enable);
        } else {
            NoClip.disable();
            alt.deleteMeta('noclip-enabled');
            alt.emitServer(AdminEvents.noclip.disable);
        }
    },
    ...
    tick() {
        ...
        if (Date.now() > nextUpdate) {
            ...
            alt.emitServer(AdminEvents.noclip.update, fwd);
        }

        drawText2D('Left Shift (Sprint Speed) | Scroll (Change Sprint Speed)', { x: 0.5, y: 0.89 }, 0.4, new alt.RGBA(255, 255, 255, 200), 0);

        drawText2D('Speed: ' + sens.toFixed(2), { x: 0.5, y: 0.92 }, 0.4, new alt.RGBA(255, 255, 255, 200), 0);
...
function drawText2D(text: string, pos: alt.IVector2, scale: number, color: alt.RGBA, alignment: number = 0, padding: number = 0) {
    if (scale > 2) {
        scale = 2;
    }
